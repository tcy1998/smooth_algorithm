...
class Interface:
    ...
    def subscriber(self):
        ...
        # self.vehicle_cmd_sub = rospy.Subscriber(
        #     "/vehicle_cmd", VehicleCmd, self.ctrl_cmd_callback
        # )
        self.vehicle_cmd_sub = rospy.Subscriber(
            "/mpc_cmd_vel", VehicleCmd, self.ctrl_cmd_callback
        )
        self.velocity_gps_sub = rospy.Subscriber(
            "/novatel/gps", GPSFix, self.velocity_gps_callback
        )
        self.velocity_meter_sub = rospy.Subscriber(
            "/pacmod/parsed_tx/vehicle_speed_rpt",
            VehicleSpeedRpt,
            self.velocity_meter_callback,
        )
    def publisher(self):
    ...
    def start(self):
        ...
        while not rospy.is_shutdown():
            ...
            if z < self.reference_velocity - 0.2 or z > self.reference_velocity + 0.2:
                expected_acceleration = speed_controller.get_control(
                    current_time, self.reference_velocity - z
                )
            ...
            stdscr.addstr(
                5, 0, "Current speed gps: %3f m/s\n" % self.current_velocity_gps
            )
            stdscr.addstr(
                6, 0, "Current speed meter: %3f m/s\n" % self.current_velocity_meter
            )
            stdscr.addstr(
                7,
                0,
        ...
        axs[0, 0].plot(
            xs, ys_cur_vel_meter_filted, alpha=0.75, label="real_meter_filted"
        )
        ...
        image_info = "\
            ...
            time=dt.datetime.now(timezone("America/Chicago")).strftime(
                "%Y-%m-%d %H:%M:%S %Z"
            ),
        )
        ...
        df.to_csv(
            "/home/autoware/Autoware/src/vehicle_interface/data_1000Hz_withValid2.csv",
            index=False,
        )
